// Web implementation - lightweight in-memory shim for web to avoid wasm requirement
type SQLiteDatabase = any;
let db: SQLiteDatabase | null = null;

/**
 * Initialize in-memory database for web platform
 */
  ...
  db = {
    ...
    runAsync: async (query: string, params: any[] = []) => {
      ...
      if (query.startsWith('INSERT INTO habits')) {
        // With icon: id, name, color, icon, created_at, updated_at
      ...
      if (query.startsWith('UPDATE habits')) {
        // Supports updates used by completeHabit and generic updates
    ...
    getFirstAsync: async (query: string, params: any[] = []) => {
      if (query.includes('FROM character_state')) {
        return memoryStore.character_state[0];
      }
      ...
      if (query.startsWith('SELECT COUNT(*) as count FROM habits WHERE completed_today = 1')) {
        const count = memoryStore.habits.filter((h) => h.completed_today === 1).length;
        return { count } as any;
      }
      ...
      if (query.startsWith('SELECT COUNT(*) as count FROM todos WHERE priority = 1 AND completed = 0')) {
        const count = memoryStore.todos.filter((t) => t.priority === 1 && t.completed === 0).length;
        return { count } as any;
      }

      if (query.startsWith('SELECT COUNT(*) as count FROM todos WHERE priority = 1 AND completed = 0 AND id !=')) {
        const [excludeId] = params;
        const count = memoryStore.todos.filter((t) => t.priority === 1 && t.completed === 0 && t.id !== excludeId).length;
        return { count } as any;
      }
    ...
    getAllAsync: async (query: string, params: any[] = []) => {
      ...
      if (query.startsWith('SELECT * FROM todos')) {
        ...
        if (query.includes('WHERE completed = 0')) {
          rows = rows.filter((t) => t.completed === 0);
        }
        if (query.includes('WHERE priority = 1 AND completed = 0')) {
          rows = rows.filter((t) => t.priority === 1 && t.completed === 0);
        }
        if (query.includes('ORDER BY priority DESC, created_at DESC')) {
          rows.sort((a, b) => (b.priority - a.priority) || (b.created_at.localeCompare(a.created_at)));
        }
        if (query.includes('ORDER BY created_at ASC')) {
          rows.sort((a, b) => a.created_at.localeCompare(b.created_at));
        }
